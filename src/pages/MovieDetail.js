...
export const MovieDetail = () => {
  ...
  useEffect( ()=>{
    async function fetchMovie(){
      const response= await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=6933da36a64f895a0eb661bd9547839d`);
          ...
          {movie.genres ?
           <p className="my-7 flex flex-wrap gap-2">
           {
             movie.genres.map( (genre)=> (
               <span className="mr-2 border border-gray-200 rounded dark:border-gray-600 p-2 hover:bg-gradient-to-r from-cyan-600 to-blue-600 ..." key={genre.id}>{genre.name}</span>
             ))
